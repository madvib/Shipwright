...
| Custom issue/category names | Partial | Custom statuses are supported in CLI/UI | Git category model is still fixed (`issues`, `releases`, `features`, `specs`, `adrs`, `notes`, `agents`, `events.ndjson`, `ship.toml`, `templates`) |
