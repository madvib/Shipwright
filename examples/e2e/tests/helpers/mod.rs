...
impl TestProject {
    ...
    fn init_with_cli(base_dir: &Path, _global_dir: &Path) -> Result<PathBuf> {
        ship_module_project::init_project(base_dir.to_path_buf())
    }
    ...
    pub fn with_fixture_project(fixture_name: &str) -> Result<Self> {
        ...
        if !fixture_root.exists() {
            anyhow::bail!("missing fixture project: {}", fixture_root.display());
        }
