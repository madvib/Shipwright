...
/// Export no longer emits provider instruction docs from mode prompt settings.
#[test]
fn gemini_export_does_not_emit_mode_instruction_docs() {
    let p = TestProject::new().unwrap();
    create_skill(
        &p.ship_dir,
        "release-gate-prompt",
    ...
    runtime::agents::export::export_to(p.ship_dir.clone(), "gemini").unwrap();
    assert!(
        !p.root().join("GEMINI.md").exists(),
        "GEMINI.md should not be generated from mode prompt settings"
    );
    ...
    assert!(
        !p.root().join("GEMINI.md").exists(),
        "GEMINI.md should remain absent after regeneration"
    );
}

/// Codex export should not emit AGENTS.md instruction output from mode prompt settings.
#[test]
fn codex_export_does_not_emit_mode_instruction_docs() {
    let p = TestProject::new().unwrap();
    create_skill(
        &p.ship_dir,
    ...
    assert!(
        !p.root().join("AGENTS.md").exists(),
        "AGENTS.md should not be generated from mode prompt settings"
    );
    ...
    assert!(
        !p.root().join("AGENTS.md").exists(),
        "AGENTS.md should remain absent after regeneration"
    );
}
