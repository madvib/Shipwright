...
function OutputSection({ compileState, selectedProviders, onCompile, hasContent }: {
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {compileState.status === 'idle' && (
          <div className="flex flex-col items-center justify-center h-full p-6 text-center">
            {hasContent ? (
              <>
                <p className="text-xs text-muted-foreground mb-3">Config ready to preview</p>
                <button onClick={onCompile} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground transition hover:bg-primary/90">
                  Compile now
                </button>
              </>
            ) : (
              <p className="text-xs text-muted-foreground">Add skills or MCP servers to see output</p>
            )}
          </div>
        )}
  ...
  return (
    <div className="p-4">
      ...
      <button
        ...
        className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-foreground px-3 py-2 text-xs font-medium text-background transition hover:opacity-90"
      >
