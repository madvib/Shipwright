import { Globe, FolderOpen, User, Unplug } from 'lucide-react'
import type { JsonValue } from '@ship/ui'
import { useDaemon } from '#/features/studio/hooks/useDaemon'
import { useSkillVars, useSetSkillVar } from '#/features/studio/mcp-queries'
import type { LibrarySkill } from './useSkillsLibrary'
import { BoolEditor, EnumEditor, StringEditor, ArrayEditor } from './var-editors'

export interface VarDef {
  type?: string
  default?: JsonValue
  'storage-hint'?: string
  values?: string[]
  label?: string
  description?: string
}

export function parseVarsSchema(schema: JsonValue | null): Record<string, VarDef> {
  if (!schema || typeof schema !== 'object' || Array.isArray(schema)) return {}
  const result: Record<string, VarDef> = {}
  for (const [key, val] of Object.entries(schema as Record<string, JsonValue>)) {
    if (key === '$schema') continue
    if (val && typeof val === 'object' && !Array.isArray(val)) {
      result[key] = val as unknown as VarDef
    }
  }
  return result
}

function scopeIcon(hint: string | undefined) {
  switch (hint) {
    case 'project': return <FolderOpen className="size-3 text-emerald-400" />
    case 'local': return <User className="size-3 text-amber-400" />
    default: return <Globe className="size-3 text-sky-400" />
  }
}

function scopeLabel(hint: string | undefined) {
  switch (hint) {
    case 'project': return 'project'
    case 'local': return 'local'
    default: return 'global'
  }
}

function formatDefault(val: JsonValue | undefined): string {
  if (val === undefined || val === null) return '--'
  if (typeof val === 'boolean') return val ? 'true' : 'false'
  if (typeof val === 'string') return val || '""'
  return JSON.stringify(val)
}

function VarCard({
  varKey,
  def,
  currentValue,
  isConnected,
  onSave,
  isPending,
}: {
  varKey: string
  def: VarDef
  currentValue: unknown
  isConnected: boolean
  onSave: (valueJson: string) => void
  isPending: boolean
}) {
  const varType = def.type ?? 'string'

  return (
    <div className="border border-border rounded-lg p-2.5 bg-card/60">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-medium text-foreground">{def.label ?? varKey}</span>
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] font-mono bg-muted px-1.5 py-0.5 rounded text-muted-foreground">
            {varType}
          </span>
          <span className="flex items-center gap-0.5" title={scopeLabel(def['storage-hint'])}>
            {scopeIcon(def['storage-hint'])}
          </span>
        </div>
      </div>
      {def.description && (
        <p className="text-[11px] text-muted-foreground mb-2 leading-relaxed">{def.description}</p>
      )}

      {isConnected ? (
        <VarEditorSwitch
          varType={varType}
          def={def}
          currentValue={currentValue}
          onSave={onSave}
          isPending={isPending}
        />
      ) : (
        <ReadOnlyValue varType={varType} def={def} />
      )}

      <div className="flex items-center gap-1 text-[10px] text-muted-foreground mt-1.5">
        {scopeIcon(def['storage-hint'])}
        <span>{scopeLabel(def['storage-hint'])}</span>
        <span className="mx-0.5">&middot;</span>
        <span className="font-mono">{varKey}</span>
      </div>
    </div>
  )
}

function VarEditorSwitch({
  varType,
  def,
  currentValue,
  onSave,
  isPending,
}: {
  varType: string
  def: VarDef
  currentValue: unknown
  onSave: (valueJson: string) => void
  isPending: boolean
}) {
  const base = { currentValue, defaultValue: def.default, onSave, isPending }

  if (varType === 'bool') {
    return (
      <div className="space-y-1">
        <BoolEditor {...base} />
        <DefaultLabel value={def.default} />
      </div>
    )
  }
  if (varType === 'enum' && def.values) {
    return (
      <div className="space-y-1">
        <EnumEditor {...base} values={def.values} />
        <DefaultLabel value={def.default} />
      </div>
    )
  }
  if (varType === 'array') {
    return (
      <div className="space-y-1">
        <ArrayEditor {...base} />
        <DefaultLabel value={def.default} />
      </div>
    )
  }
  return (
    <div className="space-y-1">
      <StringEditor {...base} />
      <DefaultLabel value={def.default} />
    </div>
  )
}

function DefaultLabel({ value }: { value: JsonValue | undefined }) {
  return (
    <div className="text-[10px] text-muted-foreground/70">
      default: <span className="font-mono">{formatDefault(value)}</span>
    </div>
  )
}

function ReadOnlyValue({ varType, def }: { varType: string; def: VarDef }) {
  if (varType === 'bool') {
    return (
      <div className="flex items-center gap-2">
        <div className={`w-7 h-4 rounded-full ${def.default === true ? 'bg-primary' : 'bg-muted'}`}>
          <div className={`size-3 rounded-full bg-white mt-0.5 transition-transform ${def.default === true ? 'translate-x-3.5' : 'translate-x-0.5'}`} />
        </div>
        <span className="text-[11px] text-muted-foreground">default: {def.default === true ? 'true' : 'false'}</span>
      </div>
    )
  }
  if (varType === 'enum' && def.values) {
    return (
      <div className="flex flex-wrap gap-1">
        {def.values.map((val) => (
          <span
            key={val}
            className={`text-[10px] px-1.5 py-0.5 rounded border ${
              val === def.default ? 'border-primary/50 bg-primary/10 text-primary font-medium' : 'border-border bg-muted text-muted-foreground'
            }`}
          >
            {val}
          </span>
        ))}
      </div>
    )
  }
  return (
    <div className="text-[11px] text-muted-foreground">
      default: <span className="font-mono">{formatDefault(def.default)}</span>
    </div>
  )
}

export function VarsTab({ skill, onAddVars }: { skill: LibrarySkill; onAddVars?: () => void }) {
  const { connected: isConnected } = useDaemon()
  const skillId = skill.stableId ?? skill.id
  const varsQuery = useSkillVars(isConnected ? skillId : null)
  const setVar = useSetSkillVar()

  const vars = parseVarsSchema(skill.varsSchema)
  const varKeys = Object.keys(vars)

  if (varKeys.length === 0) {
    return (
      <div className="py-8 text-center">
        <p className="text-xs text-muted-foreground">No variables defined.</p>
        <p className="text-[11px] text-muted-foreground mt-2">
          Variables make this a smart skill — same skill, personalized output.
        </p>
        {onAddVars && (
          <button
            onClick={onAddVars}
            className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border bg-muted/50 text-xs font-medium text-foreground hover:bg-muted transition-colors"
          >
            Add vars.json
          </button>
        )}
      </div>
    )
  }

  const mergedValues = varsQuery.data ?? {}

  return (
    <div className="space-y-3">
      {!isConnected && (
        <div className="flex items-center gap-1.5 px-2 py-1.5 rounded bg-muted/60 border border-border text-[11px] text-muted-foreground">
          <Unplug className="size-3 shrink-0" />
          <span>Connect to daemon to edit variables</span>
        </div>
      )}

      {varKeys.map((key) => (
        <VarCard
          key={key}
          varKey={key}
          def={vars[key]}
          currentValue={mergedValues[key]}
          isConnected={isConnected}
          onSave={(valueJson) => setVar.mutate({ skillId, key, valueJson })}
          isPending={setVar.isPending}
        />
      ))}
    </div>
  )
}
