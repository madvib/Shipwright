  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        {mode === 'browse' ? (
          <BrowsePanel available={available} onAdd={addPopular} />
        ) : (
          <CustomForm
...
function BrowsePanel({ available, onAdd }: {
      ...
      <div className="flex gap-4 items-end">
        <div className="flex-1">
          ...
          <Input
            ...
            min={1}
            value={p.timeout}
            onChange={(e) => p.setTimeout(e.target.value)}
            placeholder="30"
          />
        </div>
        <label className="flex items-center gap-2 pb-2 cursor-pointer select-none">
          <input
            type="checkbox"
            checked={p.disabled}
            onChange={(e) => p.setDisabled(e.target.checked)}
