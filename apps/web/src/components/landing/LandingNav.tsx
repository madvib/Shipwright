    ...
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-border/40 bg-background/85 backdrop-blur-xl">
      ...
      {mobileOpen && (
        <div className="border-t border-border/40 bg-background px-6 pb-4 pt-2 sm:hidden">
          ...
          {!user && (
            <button
              onClick={() => { setMobileOpen(false); void authClient.signIn.social({ provider: 'github', callbackURL: '/studio' }) }}
              className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg border border-border bg-transparent px-3.5 py-2 text-sm text-muted-foreground transition hover:border-border/80 hover:text-foreground"
            >
