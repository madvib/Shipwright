    ...
    match command {
        ...
        Some(cmd) => match cmd {
            ...
            Commands::Sync { pull: _, push: _ } => {
                stub("sync", "Cloud sync requires a Ship account. Run: ship-studio login")
            }
            Commands::Server { .. } => {
    ...
    if global {
        ...
        if !gitignore.exists() {
            std::fs::write(&gitignore,
                "# Ship Studio — compiled artifacts (regenerated by ship-studio compile)\n\
                 /secrets/\nCLAUDE.md\nGEMINI.md\nAGENTS.md\n.mcp.json\n.codex/\n.gemini/\n.cursor/\n"
            )?;
        }
        // README in .ship/
        ...
        if !readme.exists() {
            std::fs::write(&readme,
                "# Ship Configuration\n\nThis project uses [Ship](https://getship.dev) \
                 ...
                 Then: `ship-studio use` to activate the project mode.\n"
            )?;
        }
        println!("✓ initialized .ship/ in current directory");
        println!("  providers: {}", provider.as_deref().unwrap_or("claude"));
        println!("\nNext steps:");
        println!("  ship-studio mode create <name>   create a mode");
        println!("  ship-studio compile               generate agent configs");
    }
    Ok(())
}
...
fn run_whoami() -> Result<()> {
    ...
    match cfg.identity {
        ...
        _ => {
            ...
            println!("Run: ship-studio init --global");
        }
    }
    Ok(())
}
...
fn run_use(mode_id: &str, path: Option<PathBuf>, do_compile: bool) -> Result<()> {
    ...
    if mode_file.is_none() {
        anyhow::bail!(
            "Mode '{}' not found.\n  Create it with: ship-studio mode create {}\n  Or list available: ship-studio modes",
            mode_id, mode_id
        );
    }
...
fn run_status(path: Option<PathBuf>) -> Result<()> {
    ...
    match ctx.active_mode_for(&target) {
        ...
        None => {
            ...
            println!("Run: ship-studio use <mode-id>");
        }
    }
    Ok(())
}
...
fn run_modes(local: bool, project: bool, cloud: bool) -> Result<()> {
    if cloud {
        println!("Cloud modes require a Ship account. Run: ship-studio login");
        return Ok(());
    }
    let modes = paths::list_mode_ids(local, project);
    if modes.is_empty() {
        println!("No modes found.");
        println!("Create one with: ship-studio mode create <name>");
    } else {
    ...
    match action {
        ...
        ModeCommands::Edit { name, editor } => {
            let path = paths::find_mode_file(&name).ok_or_else(|| {
                anyhow::anyhow!("Mode '{}' not found. Create with: ship-studio mode create {}", name, name)
            })?;
            let editor = editor
        ...
        ModeCommands::Publish { name: _ } => {
            stub("mode publish", "Publishing requires a Ship account. Run: ship-studio login")?;
        }
    }
    Ok(())
}
...
fn run_compile_cmd(
    ...
    if !project_root.join(".ship").exists() {
        anyhow::bail!(".ship/ not found. Run: ship-studio init");
    }
...
fn dispatch_skill(action: SkillCommands) -> Result<()> {
    match action {
        ...
        SkillCommands::Publish { .. } => {
            stub("skill publish", "Publishing requires a Ship account. Run: ship-studio login")
        }
    }
}
