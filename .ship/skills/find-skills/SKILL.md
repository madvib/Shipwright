...
**Always install via `ship skill add`, not `npx skills add`.** The `npx skills` CLI installs to `.agents/skills/` which is invisible to `ship use` and doesn't get compiled into provider config. Ship's skill system lives at `.ship/skills/`.
...
This installs to `.ship/skills/` and is picked up automatically by `ship use`. Do NOT use `npx skills add` — it installs to `.agents/skills/` which is a different ecosystem (Codex/Gemini) and not compiled by Ship.
