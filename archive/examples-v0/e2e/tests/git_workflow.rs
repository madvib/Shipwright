...
mod new_project {
    ...
    fn init_is_idempotent() {
        ...
        // Re-init
        let ship_dir = ship_module_project::init_project(p.root().to_path_buf()).unwrap();
        assert_eq!(ship_dir, p.ship_dir);
...
mod core_loop {
    ...
    fn full_core_loop_new_project() {
        ...
        assert_eq!(feature_workspace.status, WorkspaceStatus::Active);
    }
