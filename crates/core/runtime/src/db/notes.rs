...
pub fn list_notes(branch: Option<&str>) -> Result<Vec<Note>> {
    let mut conn = open_db()?;
...
mod tests {
    ...
    fn test_update_note() {
        let (_tmp, _ship_dir) = setup();
        let note = create_note("Draft", "initial", vec![], None).unwrap();
        update_note(&note.id, Some("Final"), None, None).unwrap();
        let got = get_note(&note.id).unwrap().unwrap();
    ...
    fn test_list_notes_filtered_by_branch() {
        let (_tmp, _ship_dir) = setup();
        create_note("Note A", "", vec![], Some("feat/x")).unwrap();
        create_note("Note B", "", vec![], Some("main")).unwrap();
        create_note("Note C", "", vec![], None).unwrap();
        let feat_notes = list_notes(Some("feat/x")).unwrap();
        assert_eq!(feat_notes.len(), 1);
        let all = list_notes(None).unwrap();
        assert_eq!(all.len(), 3);
    }
    ...
    fn test_delete_note() {
        let (_tmp, _ship_dir) = setup();
        let note = create_note("Temp", "", vec![], None).unwrap();
        delete_note(&note.id).unwrap();
        assert!(get_note(&note.id).unwrap().is_none());
    }

    #[test]
    fn test_mark_synced() {
        let (_tmp, _ship_dir) = setup();
        let note = create_note("Syncable", "body", vec![], None).unwrap();
        assert!(note.synced_at.is_none());
        mark_synced(&note.id).unwrap();
        let got = get_note(&note.id).unwrap().unwrap();
    ...
    fn test_note_and_adr_independence_on_delete() {
        ...
        assert_eq!(crate::db::adrs::list_adrs().unwrap().len(), 1);
