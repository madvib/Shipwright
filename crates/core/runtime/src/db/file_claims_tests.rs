...
fn test_claim_files_basic() {
    let (_tmp, sd) = setup();
    let job = mkjob(&sd);
    claim_files(&sd, &job, None, &["src/lib.rs", "src/main.rs"]).unwrap();

    let claims = list_claims(&sd, Some(&job)).unwrap();
...
fn test_claim_files_with_workspace_id() {
    let (_tmp, sd) = setup();
    let job = mkjob(&sd);
    claim_files(&sd, &job, Some("ws-1"), &["README.md"]).unwrap();
...
fn test_idempotent_reclaim() {
    let (_tmp, sd) = setup();
    let job = mkjob(&sd);
    claim_files(&sd, &job, None, &["src/lib.rs"]).unwrap();
...
fn test_check_conflicts_returns_all() {
    let (_tmp, sd) = setup();
    let job_a = mkjob(&sd);
    claim_files(&sd, &job_a, None, &["a.rs", "b.rs"]).unwrap();

    let conflicts = check_conflicts(&sd, &["a.rs", "b.rs", "c.rs"]).unwrap();
...
fn test_release_and_reclaim() {
    ...
    claim_files(&sd, &job_a, None, &["src/lib.rs", "src/main.rs"]).unwrap();
    let released = release_claims(&sd, &job_a).unwrap();
    assert_eq!(released, 2);

    // After release, job_b can claim the same paths.
    claim_files(&sd, &job_b, None, &["src/lib.rs", "src/main.rs"]).unwrap();
    let claims = list_claims(&sd, Some(&job_b)).unwrap();
    assert_eq!(claims.len(), 2);
}
...
fn test_list_claims_all() {
    ...
    claim_files(&sd, &job_a, None, &["a.rs"]).unwrap();
    claim_files(&sd, &job_b, None, &["b.rs"]).unwrap();

    let all = list_claims(&sd, None).unwrap();
    assert_eq!(all.len(), 2);

    let a_only = list_claims(&sd, Some(&job_a)).unwrap();
...
fn test_claim_empty_paths_is_noop() {
    let (_tmp, sd) = setup();
    let job = mkjob(&sd);
    claim_files(&sd, &job, None, &[]).unwrap();
    let claims = list_claims(&sd, Some(&job)).unwrap();
...
fn test_release_nonexistent_returns_zero() {
    let (_tmp, sd) = setup();
    let released = release_claims(&sd, "no-such-job").unwrap();
