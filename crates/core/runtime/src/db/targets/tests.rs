...
fn update_target_body_markdown_and_phase() {
    ...
    update_target(&ship_dir, &t.id, TargetPatch {
    ...
    let fetched = get_target(&ship_dir, &t.id).unwrap().unwrap();
    assert_eq!(fetched.body_markdown.as_deref(), Some("# v0.1.0\n\nShip in every project."));
...
fn update_target_file_scope() {
    ...
    update_target(&ship_dir, &t.id, TargetPatch {
        file_scope: Some(vec!["crates/core/compiler/".to_string(), "packages/compiler/".to_string()]),
        ..Default::default()
    }).unwrap();

    let fetched = get_target(&ship_dir, &t.id).unwrap().unwrap();
    assert_eq!(fetched.file_scope, vec!["crates/core/compiler/", "packages/compiler/"]);
}

#[test]
fn update_target_preserves_unset_fields() {
    let (_tmp, ship_dir) = setup();
    let t = create_target(&ship_dir, "surface", "studio", Some("Web compiler"), Some("ship in browser"), None).unwrap();
...
fn update_capability_new_fields_round_trip() {
    ...
    update_capability(&ship_dir, &c.id, CapabilityPatch {
        phase: Some("bootstrap".to_string()),
        acceptance_criteria: Some(vec!["ship workspace create works".to_string(), "worktree created".to_string()]),
    ...
    assert_eq!(fetched.acceptance_criteria, vec!["ship workspace create works", "worktree created"]);
...
fn list_capabilities_phase_filter() {
    ...
    update_capability(&ship_dir, &c1.id, CapabilityPatch {
        phase: Some("bootstrap".to_string()), ..Default::default()
    }).unwrap();
    update_capability(&ship_dir, &c2.id, CapabilityPatch {
        phase: Some("polish".to_string()), ..Default::default()
    }).unwrap();
...
fn capability_priority_ordering() {
    ...
    update_capability(&ship_dir, &c1.id, CapabilityPatch { priority: Some(10), ..Default::default() }).unwrap();
    update_capability(&ship_dir, &c2.id, CapabilityPatch { priority: Some(1), ..Default::default() }).unwrap();
