...
fn create_and_get_target() {
    ...
    let fetched = get_target(&t.id).unwrap().unwrap();
...
fn list_targets_by_kind() {
    let (_tmp, _ship_dir) = setup();
    create_target("milestone", "v0.1.0", None, None, None).unwrap();
    create_target("surface", "compiler", None, None, None).unwrap();
    let milestones = list_targets(Some("milestone")).unwrap();
...
fn update_target_body_markdown_and_phase() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("milestone", "v0.1.0", None, None, None).unwrap();
...
fn update_target_file_scope() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("surface", "compiler", None, None, None).unwrap();
...
fn capability_lifecycle() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("surface", "compiler", None, None, None).unwrap();
    let c = create_capability(&t.id, "Profile compilation", None).unwrap();
    assert_eq!(c.status, "aspirational");
    assert_eq!(c.priority, 0);
    assert!(c.acceptance_criteria.is_empty());

    mark_capability_actual(&c.id, "test: profile_scaffold_parses").unwrap();
    let caps = list_capabilities(Some(&t.id), Some("actual"), None).unwrap();
...
fn update_capability_new_fields_round_trip() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("milestone", "v0.1.0", None, None, None).unwrap();
    let c = create_capability(&t.id, "CLI workspace create", None).unwrap();
    ...
    let fetched = get_capability(&c.id).unwrap().unwrap();
...
fn update_capability_status_in_progress() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("surface", "cli", None, None, None).unwrap();
    let c = create_capability(&t.id, "ship init", None).unwrap();
    assert_eq!(c.status, "aspirational");
...
fn list_capabilities_phase_filter() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("milestone", "v0.1.0", None, None, None).unwrap();
    let c1 = create_capability(&t.id, "init", None).unwrap();
    let c2 = create_capability(&t.id, "auth", None).unwrap();
    ...
    let bootstrap = list_capabilities(Some(&t.id), None, Some("bootstrap")).unwrap();
    assert_eq!(bootstrap.len(), 1);
    assert_eq!(bootstrap[0].id, c1.id);

    let all = list_capabilities(Some(&t.id), None, None).unwrap();
    assert_eq!(all.len(), 2);
}
...
fn list_capabilities_all_filters() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("milestone", "v0.1.0", None, None, None).unwrap();
    create_capability(&t.id, "accounts", None).unwrap();
    let c2 = create_capability(&t.id, "cli auth", None).unwrap();
    mark_capability_actual(&c2.id, "ship login works").unwrap();

    let all = list_capabilities(Some(&t.id), None, None).unwrap();
    assert_eq!(all.len(), 2);
    let actual = list_capabilities(None, Some("actual"), None).unwrap();
...
fn delete_capability_removes_row() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("surface", "cli", None, None, None).unwrap();
    let c = create_capability(&t.id, "ephemeral cap", None).unwrap();

    assert!(get_capability(&c.id).unwrap().is_some());
    let deleted = delete_capability(&c.id).unwrap();
    assert!(deleted);
    assert!(get_capability(&c.id).unwrap().is_none());

    // Second delete returns false (not found)
    let deleted_again = delete_capability(&c.id).unwrap();
...
fn delete_capability_nonexistent_returns_false() {
    let (_tmp, _ship_dir) = setup();
    let deleted = delete_capability("nonexistent_id").unwrap();
    assert!(!deleted);
}
...
fn capability_priority_ordering() {
    let (_tmp, _ship_dir) = setup();
    let t = create_target("surface", "cli", None, None, None).unwrap();
    let c1 = create_capability(&t.id, "low priority", None).unwrap();
    let c2 = create_capability(&t.id, "high priority", None).unwrap();
    ...
    let caps = list_capabilities(Some(&t.id), None, None).unwrap();
