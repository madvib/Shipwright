use crate::compile::{CURSOR_PERMISSIVE_ALLOW, compile};
...
fn cursor_cli_permissions_deny_only_emits_patch() {
    ...
    let patch = out
        ...
        .expect("deny patterns must emit cursor cli patch");
    let deny = patch["permissions"]["deny"].as_array().unwrap();
    assert!(
        deny.iter().any(|v| v == "Shell(rm -rf *)"),
        "Bash(cmd) → Shell(cmd)"
    );
    assert!(
        deny.iter().any(|v| v == "Mcp(*:delete*)"),
        "mcp__*__delete* → Mcp(*:delete*)"
    );
    assert!(
        patch["permissions"].get("allow").is_none(),
        "allow=[*] must not emit allow field"
    );
}
...
fn cursor_cli_permissions_explicit_allow_translates() {
    ...
    let patch = out
        .cursor_cli_permissions
        .expect("explicit allow must emit cursor cli patch");
    let allow = patch["permissions"]["allow"].as_array().unwrap();
    assert!(allow.iter().any(|v| v == "Read(*)"), "Read → Read(*)");
    assert!(
        allow.iter().any(|v| v == "Shell(git *)"),
        "Bash(git *) → Shell(git *)"
    );
    assert!(
        allow.iter().any(|v| v == "Mcp(ship:*)"),
        "mcp__ship__* → Mcp(ship:*)"
    );
}
...
fn cursor_version_1_in_cli_permissions() {
    ...
    let patch = out
        .cursor_cli_permissions
        .expect("deny must emit cursor cli permissions");
    assert_eq!(
        patch["version"], 1,
        "cursor cli permissions must include version: 1"
    );
}
...
fn cursor_rule_alwaysapply_false_in_frontmatter() {
    let r = ResolvedConfig {
        rules: vec![Rule {
            file_name: "conditional.md".into(),
            content: "Only when relevant.".into(),
            always_apply: false,
            globs: vec![],
            description: None,
        }],
...
fn cursor_rule_globs_in_frontmatter() {
    let r = ResolvedConfig {
        rules: vec![Rule {
            file_name: "rust-only.md".into(),
            content: "Rust conventions.".into(),
            always_apply: false,
            globs: vec!["**/*.rs".into(), "Cargo.toml".into()],
            description: None,
        }],
        ..resolved(vec![])
    };
...
fn cursor_rule_description_in_frontmatter() {
    let r = ResolvedConfig {
        rules: vec![Rule {
            file_name: "smart.md".into(),
            content: "Apply intelligently.".into(),
            always_apply: false,
            globs: vec![],
            description: Some("Apply when writing React components".into()),
        }],
        ..resolved(vec![])
    };
    let out = compile(&r, "cursor").unwrap();
