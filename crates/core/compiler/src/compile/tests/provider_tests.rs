...
fn context_file_is_none_when_all_rule_content_empty() {
    let r = ResolvedConfig {
        rules: vec![make_rule("blank.md", ""), make_rule("whitespace.md", "   \n  ")],
        ..resolved(vec![])
    };
    ...
    for desc in list_providers() {
        ...
        if flags.supports_mcp {
            assert!(
                ...
                "provider {} (supports_mcp=true) must include ship server", desc.id
            );
        } else {
            assert!(
                out.mcp_servers.as_object().is_some_and(|o| o.is_empty()),
                "provider {} (supports_mcp=false) must not emit MCP entries", desc.id
            );
            assert!(out.mcp_config_path.is_none(), "provider {} must not emit mcp_config_path", desc.id);
        }
    }
}
...
fn plugins_manifest_empty_provider_defaults_to_compile_target() {
    let r = ResolvedConfig {
        plugins: PluginsManifest {
            install: vec![PluginEntry { id: "my-plugin@registry".into(), provider: String::new() }],
