...
fn gemini_policy_deny_translates_to_deny_decision() {
    ...
    let toml_str = out
        .gemini_policy_patch
        .expect("deny must emit gemini policy patch");
    let parsed: toml::Table = toml::from_str(&toml_str).expect("must be valid TOML");
    let policies = parsed["tool_policies"]
        .as_array()
        .expect("must have tool_policies array");
...
fn gemini_policy_ask_translates_to_ask_user() {
    ...
    let toml_str = out
        .gemini_policy_patch
        .expect("ask must emit gemini policy patch");
...
fn gemini_policy_explicit_allow_translates_to_allow_decision() {
    ...
    let toml_str = out
        .gemini_policy_patch
        .expect("explicit allow must emit gemini policy patch");
    let parsed: toml::Table = toml::from_str(&toml_str).expect("valid TOML");
    let policies = parsed["tool_policies"].as_array().unwrap();
    assert_eq!(policies.len(), 1);
    assert_eq!(policies[0]["tool"].as_str().unwrap(), "shell");
    ...
    let pattern = policies[0]["pattern"].as_str().unwrap();
    assert!(
        pattern.contains("git"),
        "pattern must encode the git command"
    );
}
...
fn gemini_policy_mcp_pattern_translated() {
    ...
    let toml_str = out
        .gemini_policy_patch
        .expect("mcp deny must emit gemini policy patch");
    let parsed: toml::Table = toml::from_str(&toml_str).expect("valid TOML");
    let policies = parsed["tool_policies"].as_array().unwrap();
    assert_eq!(policies[0]["tool"].as_str().unwrap(), "mcp");
    let pattern = policies[0]["pattern"].as_str().unwrap();
    assert!(
        pattern.contains("github"),
        "mcp pattern must include server name"
    );
    assert!(
        pattern.contains("delete_issue"),
        "mcp pattern must include tool name"
    );
}
...
fn gemini_policy_file_and_web_tool_mapping() {
    ...
    let toml_str = out
        .gemini_policy_patch
        .expect("must emit gemini policy patch");
    let parsed: toml::Table = toml::from_str(&toml_str).expect("valid TOML");
    let policies = parsed["tool_policies"].as_array().unwrap();
    assert_eq!(policies.len(), 3);
    let tools: Vec<&str> = policies
        .iter()
...
fn gemini_policy_output_is_valid_toml() {
    ...
    let parsed: toml::Table = toml::from_str(&toml_str).expect("must be valid TOML");
    assert!(
        parsed.contains_key("tool_policies"),
        "must use [[tool_policies]] array"
    );
    let policies = parsed["tool_policies"].as_array().unwrap();
...
fn gemini_policy_glob_star_converts_to_regex_dotstar() {
    ...
    assert!(
        !pattern.contains(" *"),
        "raw glob space-star must not remain"
    );
}
